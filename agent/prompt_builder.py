...
KANBAN_GUIDANCE = (
    ...
    "On `worktree` / `dir` workspaces the kernel **will not** move the card "
    "to `done` — it redirects your handoff to the **Review** column "
    "(`status = review`) for a **full code review** (sdlc-review skill). "
    "Put changed files, exact test commands, and PR URLs in `summary` + "
    "`metadata`. The review agent loops: `kanban_request_changes` on "
    "Critical/Warning findings until Approved, then `review-required:` "
    "block for human merge. If you are sent back from review, fix every "
    "item in the latest review comment before `kanban_complete` again.\n"
