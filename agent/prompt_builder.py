...
KANBAN_GUIDANCE = (
    ...
    "(`status = review`) for an automated SDLC review pass. Put changed "
    "files, test results, and PR URLs in `summary` + `metadata`. After "
    "automated review, the review agent blocks with `review-required:` for "
    "human sign-off, or returns the card to `ready` via "
    "`kanban_request_changes` if fixes are needed.\n"
