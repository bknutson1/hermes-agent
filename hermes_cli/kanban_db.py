            ...
            with write_txn(conn):
                ...
                conn.execute(
                    ...
                    INSERT INTO tasks (
                        ...
                        skills, max_retries, goal_mode, goal_max_turns, session_id, create_pr
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        task_id,
...
def decompose_triage_task(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        default_ws_kind, default_ws_path, default_ws_branch, default_ws_base = (
            _workspace_for_decomposed_children(root_row, task_id=task_id)
        )
        ...
        for idx, child in enumerate(children):
            ...
            # Per-child override wins; otherwise inherit decomposed defaults.
            child_ws_kind = child.get("workspace_kind") or default_ws_kind
            if child.get("workspace_path"):
                child_ws_path = child.get("workspace_path")
            elif child_ws_kind == default_ws_kind:
                child_ws_path = default_ws_path
            else:
                child_ws_path = None
            child_branch = child.get("branch_name") or (
                default_ws_branch if child_ws_kind == default_ws_kind else None
            )
            child_base = child.get("base_branch") or (
                default_ws_base if child_ws_kind == default_ws_kind else None
            )
            conn.execute(
                "INSERT INTO tasks "
